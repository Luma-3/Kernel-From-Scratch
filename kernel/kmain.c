...
void draw_apple_vbe(uint32_t start_x, uint32_t start_y) {
    ...
    // 'R' = rouge, 'r' = rouge moyen, 'D' = rouge foncé/bordeaux, 'P' = rose
    // clair (reflet)
    const char apple[16][17] = {
        "                ", "        B       ", "       BB       ",
        "       B        ", "     RRRRRR     ", "   RRRRRRRRRR   ",
        "  RRRRRRRRRRRR  ", "  RRRRRRRRRRRR  ", "  RRRRRRRRRRRR  ",
        "  RRRRRRRRRRRR  ", "  RRRRRRRRRRRR  ", "   RRRRRRRRRR   ",
        "   RRRRRRRRRR   ", "    RRRRRRRR    ", "     RRRRRR     ",
    };

    // Dessiner la pomme pixel par pixel avec scaling 4x pour qu'elle soit
    // visible
    ...
    for (uint32_t y = 0; y < 16; y++) {
        for (uint32_t x = 0; x < 16; x++) {
            ...
            case 'P':
                ...
                break;
            case ' ':
                continue; // Ne rien dessiner pour les espaces
            default:
                continue;
            }

            // Dessiner un bloc de scale x scale pixels pour chaque pixel du
            // sprite
            for (uint32_t sy = 0; sy < scale; sy++) {
                for (uint32_t sx = 0; sx < scale; sx++) {
                    terminal_put_pixel(start_x + x * scale + sx,
                                       start_y + y * scale + sy, color);
                }
            }
        }
    }
...
void draw_potato_vbe(uint32_t start_x, uint32_t start_y) {
    ...
    const char potato[16][17] = {
        "                ", "                ", "                ",
        "                ", "        BBBB    ", "     BBBDLLDB   ",
        "    BDLLLDDDBB  ", "   BLLLLLLLLDD  ", "  BDLLLLLDDDBD  ",
        "  BDLLLLLDDDBD  ", "  DLLLLDDDBBD   ", "   DLLLBBBDD    ",
        "    DDDDDD      "};
    const uint32_t scale = 4;
    for (uint32_t y = 0; y < 16; y++) {
        for (uint32_t x = 0; x < 16; x++) {
...
void draw_saturn_vbe(uint32_t start_x, uint32_t start_y) {
    ...
    const char saturn[16][17] = {
        "                ", "      DYYD      ", "    DDYYYYDD    ",
        "   DYYYYYYYYY   ", "  YOOOOOOOOOOY  ", " YBBBBBBBBBBBBY ",
        " YOOOOOOOOOOOOY ", "RMRRRRMRRRLRRRML", "LRRRRLRRRRRMRRRM",
        " YOOOOOOOOOOOOY ", " YBBBBBBBBBBBBY ", "  YOOOOOOOOOOY  ",
        "   DYYYYYYYYY   ", "    DDYYYYDD    ", "      DYYD      ",
        "                "};
...
void draw_axelote_on_bucket_vbe(uint32_t start_x, uint32_t start_y) {
    ...
    // ' ' = vide, 'D' = rose foncé, 'P' = rose, 'L' = rose clair, 'A' = rose
    // pâle, 'B' = bleu, 'b' = bleu foncé, 'G' = gris foncé, 'g' = gris clair,
    // 'l' = gris très clair , 'V' = violet foncé, "R" = magenta clair
    const char axolote[16][17] = {
        "    D      D    ", "  P PD    DP P  ", "  DPLAAAAAALPD  ",
        "   DVAALLAAVD   ", "  DDLAAAAAALDD  ", "  GbRRRRRRRRbG  ",
        "  GBGRBBBgRGGG  ", "  GlgRGbBGRggG  ", "  GlDADgBDADgG  ",
        "  GlgDggggDggG  ", "  GglllgBggggG  ", "  GlllgggggggG  ",
        "   GglggggggG   ", "   GggggggggG   ", "    GglggggG    ",
        "     GGGGGG     "};

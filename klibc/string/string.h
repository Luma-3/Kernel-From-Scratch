 ...
 * These functions are designed to be simple and efficient, suitable for use in
 * a kernel environment. They do not perform any dynamic memory allocation and
 * assume that the input strings are properly null-terminated.
 */
 ...
 * @return A pointer to the beginning of the located substring, or NULL if the
 * substring is not found within the first n characters of haystack.
 */
 ...
 * @param c The character to search for (passed as an int, but interpreted as a
 * char).
 ...
 * base and stores the result in the buffer pointed to by str.
 * @param value The integer value to convert.
 * @param str The buffer where the resulting string will be stored. It must be
 * large enough to hold the resulting string, including the null terminator.
 * @return The number of characters written to the buffer, excluding the null
 * terminator. If an error occurs (e.g., if str is NULL), the function returns
 * 0.
 */
